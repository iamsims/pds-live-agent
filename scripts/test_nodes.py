...
**Blind eval**: queries are loaded from `pds_node_classification_blind.xlsx`
(no Expected Identifiers, no node column). Node routing comes from a sidecar
read of the original file that touches only the Primary PDS Node column.
Expected Identifiers is never loaded into memory or written to results, so
the trace and xlsx outputs can be reviewed without overfitting to gold.
Each result row carries `row_index` for off-agent scoring against the
original workbook after the run completes.
...
# Blind eval: agent input is the sanitised workbook (row_index, Paper Short,
# Paper, Query — NO Expected Identifiers, NO node column). Node assignment is
# pulled from a sidecar lookup that touches only the Primary PDS Node column
# of the original file; Expected Identifiers is never read.
_BLIND_FILE = (
    Path(__file__).resolve().parent
    / "data"
    / "pds_node_classification_blind.xlsx"
)
_NODE_ASSIGNMENT_FILE = (
    Path(__file__).resolve().parent
    / "data"
    / "pds_node_classification.xlsx"
)
# Column indices in the BLIND sheet (0-based, values_only=True)
_BL_ROW_INDEX = 0
_BL_PAPER_SHORT = 1
_BL_PAPER = 2
_BL_QUERY = 3
# Primary PDS Node lives in column E (1-based 5) of the original file. We
# read this single column via cell access so Expected Identifiers (column D)
# is never touched.
_NODE_COL_1BASED = 5


def _load_node_assignments() -> dict[int, str]:
    """Return {row_index: primary_node} by reading only the node column.

    Does NOT load Expected Identifiers. This is the sole bridge between the
    blind queries (which carry row_index) and the per-node routing.
    """
    wb = openpyxl.load_workbook(_NODE_ASSIGNMENT_FILE, read_only=True)
    ws = wb["PDS Node Classification"]
    out: dict[int, str] = {}
    for i, row_num in enumerate(range(2, ws.max_row + 1), start=1):
        cell = ws.cell(row=row_num, column=_NODE_COL_1BASED).value
        out[i] = str(cell or "").strip().lower()
    wb.close()
    return out


def load_queries_for_node(node: str, limit: int | None = None) -> list[dict]:
    """Load queries for a node from the BLIND workbook.

    Filters by node assignment via the sidecar lookup (node column only).
    The returned dicts carry `row_index` so results can be joined back to the
    original file off-agent, *after* the run completes, for scoring.
    `expected_ids` is never present.
    """
    node_map = _load_node_assignments()
    target = node.strip().lower()
    wb = openpyxl.load_workbook(_BLIND_FILE, read_only=True)
    ws = wb["PDS Node Classification"]
    rows: list[dict] = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        row_index = row[_BL_ROW_INDEX]
        if row_index is None:
            continue
        primary = node_map.get(int(row_index), "")
        if primary != target:
            continue
        rows.append(
            {
                ...
                "rank": None,
                "paper_short": row[_BL_PAPER_SHORT],
                "rationale": None,
                "paper": row[_BL_PAPER],
                "query": row[_BL_QUERY],
                "node": primary,
...
# ---------------------------------------------------------------------------
# Blind results writer (no Expected Identifiers column — row_index instead)
# ---------------------------------------------------------------------------

_BLIND_RESULT_HEADERS = [
    "row_index",
    "Paper Short",
    "Paper",
    "Query",
...
def _write_blind_results_xlsx(path: Path, results: list[dict]) -> None:
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Eval Results"
    header_font = Font(bold=True)
    for col, header in enumerate(_BLIND_RESULT_HEADERS, 1):
        ws.cell(row=1, column=col, value=header).font = header_font
    wrap = Alignment(wrap_text=True, vertical="top")
    for row_idx, row_data in enumerate(results, 2):
        for col_idx, key in enumerate(_BLIND_RESULT_KEYS, 1):
            ws.cell(row=row_idx, column=col_idx, value=row_data.get(key, "")).alignment = wrap
    for col_idx, header in enumerate(_BLIND_RESULT_HEADERS, 1):
...
async def _run_node(
    ...
    async def _process(idx: int, row: dict) -> dict:
        async with sem:
            ...
            try:
                output, trace = await run_query("live", query, node=node)
                tool_calls = (
                ...
                if print_tool_calls and tool_calls:
                    print_tool_calls(tool_calls, f"{node}/{idx + 1}")
                failed = False
            ...
            return {
                **row,
                ...
                "live_candidates": format_candidates(output) if output else "ERROR",
    ...
    _write_blind_results_xlsx(node_dir / "results.xlsx", public_results)
