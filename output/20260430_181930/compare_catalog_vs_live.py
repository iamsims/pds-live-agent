...
# ── PDS3 ↔ PDS4 equivalence map ──────────────────────────────────────────────
# Maps PDS3 dataset IDs to their PDS4 bundle LIDs (same underlying dataset)
PDS3_TO_PDS4 = {
    "MRO-M-CRISM-5-RDR-MULTISPECTRAL-V1.0":  "urn:nasa:pds:mro_crism_mrdr",
    "MRO-M-CRISM-5-RDR-MPTARGETED-V1.0":     "urn:nasa:pds:mro_crism_mtrdr",
    "MRO-M-CRISM-3-RDR-TARGETED-V1.0":        "urn:nasa:pds:mro_crism_trdr",
    "MRO-M-CRISM-5-RDR-VNIRHYPERSPECTRAL-V1.0": "urn:nasa:pds:mro_crism_vrdr",
    "MEX-M-HRSC-5-REFDR-DTM-V1.0":            "urn:nasa:pds:mex_hrsc_refdr_dtm",
    "MEX-M-HRSC-5-REFDR-MAPPROJECTED-V1.0":   "urn:nasa:pds:mex_hrsc_refdr_mapprojected",
    "MEX-M-HRSC-5-REFDR-MAPPROJECTED-V3.0":   "urn:nasa:pds:mex_hrsc_refdr_mapprojected",
    "MGN-V-RDRS-5-GVDR-V1.0":                 "urn:nasa:pds:magellan_gvdr",
    "MGN-V-RDRS-5-GDR-EMISSIVITY-V1.0":       "urn:nasa:pds:magellan_gxdr",
    "MGN-V-RDRS-5-SCVDR-V1.0":                "urn:nasa:pds:magellan_scvdr",
}
# Build reverse map too
PDS4_TO_PDS3 = {}
for k, v in PDS3_TO_PDS4.items():
    PDS4_TO_PDS3.setdefault(v, []).append(k)


def normalize_pds_id(raw_id):
    """Strip version suffixes (::1.0), collection suffixes (:data*), and uppercase."""
    s = raw_id.strip()
    # Remove trailing version like ::1.0, ::1.1, ::3.0
    s = re.sub(r'::[\d.]+$', '', s)
    # Remove trailing collection segment like :data, :data_mrdr, :data_edf
    s = re.sub(r':data[_a-zA-Z0-9]*$', '', s)
    return s.upper()
...
def match_expected_ids(expected_str, found_str):
    ...
    Handles common mismatch patterns:
      1. Version suffix: expected has ::1.0 but found omits it (or vice versa)
      2. Bundle vs Collection: expected is bundle LID, found is collection LID (:data)
      3. PDS3 vs PDS4: expected is PDS3 format, found is the PDS4 equivalent
      4. Version differences: MEX-M-HRSC-5-REFDR-MAPPROJECTED-V1.0 vs V3.0
    """
    ...
    # Also index PDS3↔PDS4 equivalents
    found_equiv = {}  # normalized equiv → (original found ID, equiv type)
    for fid in found_ids:
        norm = normalize_pds_id(fid)
        for pds3, pds4 in PDS3_TO_PDS4.items():
            if norm == pds3.upper():
                found_equiv[pds4.upper()] = (fid, "PDS3→PDS4")
            if norm == pds4.upper():
                found_equiv[pds3.upper()] = (fid, "PDS4→PDS3")
    ...
    for eid in expected_ids:
        ...
        # 2) Normalized match (strips ::version and :data suffix)
        if norm_eid in found_norm_to_raw:
            matched_raw = found_norm_to_raw[norm_eid]
            if matched_raw.strip() == raw_eid:
                matches.append((True, f"Exact match: {raw_eid}"))
            else:
                # Figure out what normalization bridged the gap
                reasons = []
                if re.search(r'::[\d.]+$', raw_eid) and not re.search(r'::[\d.]+$', matched_raw):
                    reasons.append("version suffix stripped")
                elif not re.search(r'::[\d.]+$', raw_eid) and re.search(r'::[\d.]+$', matched_raw):
                    reasons.append("version suffix stripped")
                if re.search(r':data[_a-zA-Z0-9]*$', raw_eid) != re.search(r':data[_a-zA-Z0-9]*$', matched_raw):
                    reasons.append("bundle↔collection equivalent")
                if not reasons:
                    reasons.append("normalized equivalent")
                matches.append((True, f"{' + '.join(reasons)}: {raw_eid} ≈ {matched_raw}"))
            continue

        # 3) PDS version difference (e.g., V1.0 vs V3.0)
        ...
        # 4) PDS3↔PDS4 equivalence
        if norm_eid in found_equiv:
            equiv_raw, equiv_type = found_equiv[norm_eid]
            matches.append((True, f"{equiv_type} equivalent: {raw_eid} ≈ {equiv_raw}"))
            continue

        # 5) PDS3↔PDS4 with version stripping combo
        if norm_eid in {k.upper() for k in PDS3_TO_PDS4}:
            pds4_equiv = PDS3_TO_PDS4.get(raw_eid, "").upper()
            if not pds4_equiv:
                # Try without version
                for pds3_key, pds4_val in PDS3_TO_PDS4.items():
                    if re.sub(r'-V[\d.]+$', '', pds3_key.upper()) == base_no_version:
                        pds4_equiv = pds4_val.upper()
                        break
            if pds4_equiv:
                for fn, fn_raw in found_norm_to_raw.items():
                    if fn.startswith(pds4_equiv):
                        matches.append((True, f"PDS3→PDS4 equivalent: {raw_eid} ≈ {fn_raw}"))
                        break
                else:
                    matches.append((False, f"No match for {raw_eid}"))
                continue

        if norm_eid in {v.upper() for v in PDS3_TO_PDS4.values()}:
            for pds3_id in PDS4_TO_PDS3.get(raw_eid.split("::")[0], []):
                if pds3_id.upper() in found_norm_to_raw:
                    matches.append((True, f"PDS4→PDS3 equivalent: {raw_eid} ≈ {found_norm_to_raw[pds3_id.upper()]}"))
                    break
            else:
                matches.append((False, f"No match for {raw_eid}"))
            continue
